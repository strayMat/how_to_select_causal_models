...
%
However, g-computation itself does not control error on both populations as highlighted in \ref{fig:toy_example}. Specific inductive biases of ML models are able to favor such regularization \cite{johansson2022generalization}. Thus, selection procedures, more elaborated than MSE are needed to favor such models. A selection procedure is defined by a risk $\ell$ evaluating the quality of a model $f$ on the observed data $O$ and a splitting strategy of the data.
%
...
The simulation has been designed to control precisely the overlap parameter. The semi-simulated datasets have been included to explore more diverse and noisy covariate distributions than simulated covariates. These simulations cover a wide range of causal settings, such ratio of causal effect to background responses, and functional links between covariates, outcome and treatment.
