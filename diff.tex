...
%DIF ADD how_to_select_latex.tex   Sun Nov 10 13:35:38 2024
...
\DIFadd{Beyond overlap, we study for caussim simulations, the effect on model selection
    ...
    absolute difference between the causal effect and the baseline, $\Delta_{\mu} = \frac{1}{N} \sum_{i=1}^N \big | \frac{\mu_{1}(x_i) - \mu_{0}(x_i)}{\mu_{0}(x_i) + \mu_{1}(x_i) - \frac{1}{N} \sum_{j=1}^N \mu_0(x_j) + \mu_1(x_j)} \big |$, covering a ratio range from 0.04 to 206 ($\text{median}=9.1$, full
