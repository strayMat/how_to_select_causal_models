...
G-computation is one choice of decomposition of the CATE estimation into a sequence of regressions, where the only statistical estimate is the response function $\mu_{a}(x)$ \cite[chapter 15]{chernozhukov2024applied}. Other choices of decomposition exist, such as the R-decomposition \cite{robinson_rootnconsistent_1988}. It introduces two new statistical estimates, the conditional mean outcome
...
\paragraph{R-risk is robust to a wide range of effect ratio}

Beyond overlap, we study the effect of varying ratio of causal effect ratio to
baseline on model selection. In our simulation, we vary the empirical mean
absolute difference between the causal effect and the baseline, $\Delta_{\mu} =
    ...
    \big|$, covering a range of ratio from 1 to 300. Figure
...
\begin{figure}[!tb]
    % XXX: for submission
    \centering\begin{minipage}{\linewidth}
        \includegraphics[width=\linewidth]{_6_effect_ratio_by_bin_comparaison_kendall_by_Dataset.pdf}
    \end{minipage}
    ...
    \caption{\textbf{R-risk is robust to a wide range of effect ratio}:
        ...
        baseline response, ranging from 1 to 300. Appendix \ref{apd:effect_ratio}
        details this simulation.}\label{fig:effect_ratio_influence}
\end{figure}
...
situations. If one possess some prior knowledge on the data generation process,
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \State $x \sim \mathbb P(X)$ \Comment{Sample from rotated gaussian mixture}
        \State  $z(x)$ \Comment{Compute the Nystroem expansion}
        \State $\mu_0(x) = \begin{bmatrix} z(x); 1 \end{bmatrix} \cdot \beta_{\mu}^T$ \Comment{Compute the non-treated response surface, ie. the baseline}
        \State  $\tau(x) = \begin{bmatrix} z(x); 1 \end{bmatrix} \cdot \beta_{\tau}^T$ \Comment{Compute the conditional treatment effect (CATE)}
        \State $y(a) = (1-\omega) \mu_0(x) + a\,\omega\tau(x) + \varepsilon(x, a) + \varepsilon(x, a)$ \Comment{Compute the potential outcomes}
        \State $a \sim \operatorname{Bernoulli}(e(x))$ \Comment{Compute the treatment status}
        \State $y = a\,y(1) + (1-a)\,y(0)$ \Comment{Compute the observed outcome}
    \end{algorithmic}
...
For each dataset of our simulation (caussim), we measure it as the absolute mean
...
ratio to cover a wide range of values. For 90 dataset instances, it ranges from 1
to 300 with the full distribution of values given in Table \ref{apd:effect_ratio_distribution}.

\begin{table}[!ht]
    \centering
    \begin{tabular}{|l|l|l|l|l|l|l|l|l|l|l|}
        \hline
        ~             & count & mean    & std     & min     & 10\%    & 25\%    & 50\%    & 75\%    & 90\%    & max     \\ \hline
        effect\_ratio & 89    & 99.1291 & 132.525 & 1.24843 & 2.82881 & 4.07361 & 30.1524 & 184.996 & 316.999 & 510.897 \\ \hline
    \end{tabular}
    \caption{Distribution of the causal effect ratio $\Delta_\mu$ for the experiment.}\label{apd:effect_ratio_distribution}
\end{table}
