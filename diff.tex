...
%DIF DEL original_manuscript.tex   Sun Sep  1 18:46:04 2024
%DIF ADD how_to_select_latex.tex   Sun Nov 10 17:53:57 2024
...
%DIF 66a66
\usepackage{algpseudocode} % for algorithmic %DIF > 
%DIF -------
...
\begin{frontmatter}
    ...
    \begin{abstract}
        ...
        \textbf{Methods:}
        We study \DIFdelbegin \DIFdel{such a }\DIFdelend \DIFaddbegin \DIFadd{a large }\DIFaddend variety of model selection procedures in practical settings:
        ...
        does not bring much improvements \DIFdelbegin \DIFdel{. The
        }\DIFdelend \DIFaddbegin \DIFadd{in most cases. On average, the
        }\DIFaddend $R\text{-risk}$, which uses as nuisances a model of
    ...
    \begin{keywords}
        Model Selection, Predictive model, Treatment Effect, \DIFdelbegin \DIFdel{G-formula}\DIFdelend \DIFaddbegin \DIFadd{G-computation}\DIFaddend , Machine Learning
    \end{keywords}
...
Predictive modeling \DIFdelbegin \DIFdel{bridges }\DIFdelend \DIFaddbegin \DIFadd{is linked }\DIFaddend to causal inference theory \DIFdelbegin \DIFdel{under the name }\DIFdelend \DIFaddbegin \DIFadd{by the concept }\DIFaddend of
\emph{outcome models} (or \DIFdelbegin \DIFdel{G-computation, G-formula }\DIFdelend \DIFaddbegin \DIFadd{g-computation, g-estimation,
    g-formula }\DIFaddend \cite{robins_role_1986}, Q-model
...
%DIF < \idea{There is a variety of models for g-estimation}
%DIF > \idea{There is a variety of models for g-computation}
...
predictive models \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
        \cite{fang2019applying,dorie_automated_2019} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
        \cite{fang2019applying,dorie_automated_2019,leborgne2021g,ren2023comparing} }\hskip0pt%DIFAUXCMD
}\DIFaddend (illustration in
...
As no single machine-learning method performs best across all \DIFdelbegin \DIFdel{data sets}\DIFdelend \DIFaddbegin \DIFadd{datasets}\DIFaddend , there is a
...
\DIFdelbegin \DIFdel{We study }\DIFdelend \DIFaddbegin \DIFadd{The intersection between machine learning and causal inference is growing rapidly
    \mbox{%DIFAUXCMD
        \cite{kaddour2022causal,chernozhukov2024applied}}\hskip0pt%DIFAUXCMD
    . We focus on }\DIFaddend \textit{model selection procedures} in practical settings,
...
framework and \DIFdelbegin \DIFdel{details }\DIFdelend \DIFaddbegin \DIFadd{detail }\DIFaddend the causal risks and model-selection procedure. We then
...
This toy example illustrates that the classic minimum \DIFdelbegin \DIFdel{Mean Square Error
}\DIFdelend \DIFaddbegin \DIFadd{mean squared error (MSE)
}\DIFaddend criterion is not suited to choosing a model among candidate
...
\DIFaddbegin \DIFadd{We first expose the classic construction of the outcome modeling (or
    G-computation) estimators of causal effect
    \mbox{%DIFAUXCMD
        \cite{robins_new_1986,snowden_implementation_2011,hernan_causal_2020}}\hskip0pt%DIFAUXCMD
    .
}

\DIFaddend \paragraph{Settings}
...
causal estimands \cite{rubin_causal_2005}. \DIFdelbegin \DIFdel{The usual
}\emph{\DIFdel{strong ignorability}} %DIFAUXCMD
\DIFdel{assumptions are  (details in
    \ref{apd:causal_assumptions}): }\DIFdelend \emph{1)} \DIFaddbegin \DIFadd{an individual's
    outcome $Y$ is solely governed by the corresponding potential outcome:
}\begin{flalign}
    \DIFadd{\text{\emph{Consistency} assumption,}
    } &  &
    \DIFadd{Y = A\, Y(1) + (1 - A)\, Y(0)
        \label{eq:consistency}
    } &  &
    \DIFadd{}\end{flalign}
\emph{\DIFadd{2)}}
\DIFaddend \emph{unconfoundedness} \mbox{$\{Y(0),
        Y(1) \} \indep A | X$}, \emph{~\DIFdelbegin \DIFdel{2}\DIFdelend \DIFaddbegin \DIFadd{3}\DIFaddend )} \emph{strong overlap} ie. every patient has a
strictly positive probability to receive each treatment,
\DIFdelbegin \emph{\DIFdel{3)}}
%DIFAUXCMD
\emph{\DIFdel{consistency}}%DIFAUXCMD
\DIFdel{, }\DIFdelend and \emph{4)} \emph{generalization} \DIFaddbegin \DIFadd{--no covariate shift.
    These classic assumptions, called
}\emph{\DIFadd{strong ignorability}}\DIFadd{, are formally detailed in
    \ref{apd:causal_assumptions}}\DIFaddend .

\paragraph{\DIFdelbegin \DIFdel{Estimating }\DIFdelend \DIFaddbegin \DIFadd{Identifying }\DIFaddend treatment effects with outcome models -- g-computation \cite{robins_new_1986}}\label{subsec:estimators}

Should we know the two \DIFdelbegin \DIFdel{expected }\DIFdelend \DIFaddbegin \DIFadd{potential }\DIFaddend outcomes for a given $X$,
we could compute the
difference between them, which gives the causal effect of the treatment.
%
These two \DIFdelbegin \DIFdel{expected }\DIFdelend \DIFaddbegin \DIFadd{potential }\DIFaddend outcomes can be estimated from observed data:
...
equality\DIFaddbegin \DIFadd{, linking the target quantity to the observed data}\DIFaddend :
...
\begin{eqnarray}
    \tau =& \mathbb E\DIFdelbegin \DIFdel{_{Y \sim \mathcal{D^{\star}}}[Y(1) - Y(0)|X=x]
    }\DIFdelend \DIFaddbegin \DIFadd{_{Y \sim \mathcal{D^{\star}}}[Y(1) - Y(0)]
    }\DIFaddend \notag
    \\
    = &\mathbb E_{Y \sim \mathcal{D}}[Y|A=1] - \mathbb E_{Y \sim \mathcal{D}}[Y| A=0]
    \label{eq:tau_population}
\end{eqnarray}
This equation builds on \DIFdelbegin \DIFdel{two quantities: the conditional expectancy
}\DIFdelend \DIFaddbegin \DIFadd{the conditional expectation }\DIFaddend of the outcome given the \DIFdelbegin \DIFdel{covariates and either
    treatment or no no treatment }\DIFdelend \DIFaddbegin \DIFadd{treatment $\mathbb E_{\sim \mathcal{D}}[Y|A]$. Outcome based methods target this quantity conditionally on the covariates}\DIFaddend , called \emph{response function}:
\begin{flalign*}
...
finite sum version of \autoref{eq:tau_population} leads to an \DIFaddbegin \DIFadd{unbiased
}\DIFaddend estimator of the ATE written:
\begin{equation}
...
\DIFaddbegin \DIFadd{The formulas in Eq. (\ref{eq:mu_identification}-\ref{eq:ate_estimate}) are all partly oracle formulas: they rely on
    ...
    %DIF > In practice the response function can be estimated with one statistical model taking the treatment as a supplementary covariate $\hat \mu(x, a)$ (called S-learner) or by two separate model fitted on each population, treated or not $(\hat \mu_0(x), \hat \mu_1(x))$ (T-learner) \cite{kunzel_metalearners_2019}.
}\DIFaddend

Similarly to the ATE, at the individual level \DIFdelbegin \DIFdel{, the CATE }\DIFdelend \DIFaddbegin \DIFadd{Eq.\ref{eq:mu_identification} links the CATE to statistical quantities}\DIFaddend :
\begin{equation}
...
\DIFdelbegin \DIFdel{The }\emph{\DIFdel{R-decomposition}}
%DIFAUXCMD
\DIFdel{of the outcome model\mbox{%DIFAUXCMD
        \cite{robinson_rootnconsistent_1988}}\hskip0pt%DIFAUXCMD
    plays an important role:
    introducing two quantities}\DIFdelend \DIFaddbegin \DIFadd{G-computation is a choice of decomposition of the CATE estimation. Other choices
    of decomposition exist, such as the R-decomposition
    \mbox{%DIFAUXCMD
        \cite{robinson_rootnconsistent_1988}}\hskip0pt%DIFAUXCMD
    . The latter introduces two new statistical
    estimates}\DIFaddend , the conditional mean outcome and the probability \DIFdelbegin \DIFdel{to be }\DIFdelend \DIFaddbegin \DIFadd{of being }\DIFaddend treated (known
...
\end{flalign}
\DIFdelbegin \DIFdel{the outcome }\DIFdelend \DIFaddbegin \DIFadd{with these, the outcome (Eq.\,\ref{eq:consistency}) }\DIFaddend can be written
...
\emph{nuisances} \cite{chernozhukov_double_2018}\DIFdelbegin \DIFdel{; they are unknown }\DIFdelend \DIFaddbegin \DIFadd{. They are unknown and must be estimated from the data.
}

\bigskip

\DIFadd{Both the in the ATE and CATE formula Eq.\,(\ref{eq:ate_estimate},
...
\subsection{Model-selection\DIFdelbegin \DIFdel{risks}\DIFdelend , oracle and feasible \DIFaddbegin \DIFadd{risks}\DIFaddend }\label{sec:problem:model_selection}


\paragraph{Causal model selection}\label{sec:problem:causal_selection}

We formalize model selection for causal estimation. Thanks to the \DIFdelbegin \DIFdel{g-formula
}\DIFdelend \DIFaddbegin \DIFadd{outcome model }\DIFaddend identification (\autoref{eq:mu_identification}), a given \DIFdelbegin \DIFdel{outcome }\DIFdelend model $f: \mathcal X
...
%
\DIFdelbegin \DIFdel{Let }\DIFdelend \DIFaddbegin \DIFadd{However, the g-computation framework presented above is written in terms
    of ``perfect'' conditional expectations (oracles), it does not control an
    error, }\emph{\DIFadd{eg}} \DIFadd{on both populations as highlighted in Figure
    \ref{fig:toy_example}. Selection procedures are needed to find the best
    conditional-expectation models.
}

\DIFadd{A selection procedure combines a risk $\ell$, evaluating the quality of a
    ...
    %DIF > 
    Formally, let }\DIFaddend $\mathcal F=\{f: \mathcal X \times \mathcal A \rightarrow \mathcal{Y}\}$ be
...
\subsection{\DIFdelbegin \DIFdel{Estimation and model }\DIFdelend \DIFaddbegin \DIFadd{Model }\DIFaddend selection procedure}\label{problem:estimation_procedure}
...
\begin{algorithm}[!htbp]
    \caption{Model selection procedure}\label{problem:estimation_procedure:algo} {%
        ...
        a candidate \DIFdelbegin \DIFdel{estimators }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend $f$, a causal
        metrics $\ell$:
        \begin{enumerate}
...
\DIFaddbegin \DIFadd{The simulations, designed to evaluate the effect of the overlap
...
\DIFaddend \subsection{Caussim: Extensive simulation settings}\label{subsec:simulations}
...
data-generating process. To build such estimators, we first use \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend RBF
...
non treated:
\begin{itemize}
    \item SLearner: A single learner for both \DIFdelbegin \DIFdel{population}\DIFdelend \DIFaddbegin \DIFadd{populations}\DIFaddend , taking the treatment as
...
\paragraph{The $R\text{-risk}$ is the best metric \DIFaddbegin \DIFadd{on average}\DIFaddend }
...
to split out a third separated \DIFdelbegin \DIFdel{data set }\DIFdelend \DIFaddbegin \DIFadd{dataset }\DIFaddend --a ``nuisance set''-- to fit the
...
\DIFaddbegin \paragraph{\DIFadd{R-risk is robust to a wide range of effect ratio values}}

\DIFadd{Beyond overlap, we study for caussim simulations, the effect on model selection
...
\begin{figure}[!tb]
    ...
    \caption{\textbf{\DIFaddFL{R-risk is robust to a wide range of effect ratio}}\DIFaddFL{:
...
\DIFaddend \section{Discussion and conclusion}\label{sec:discussion}\label{sec:conclusion}
...
to more valid causal estimates \DIFaddbegin \DIFadd{on average}\DIFaddend . Estimating the $R\text{-risk}$ requires a more
...
putative intervention, the $R\text{-risk}$. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{On average, the }\DIFaddend $R\text{-risk}$
...
%DIF > 
\DIFaddbegin \DIFadd{Our conclusions are that without prior knowledge, the R-risk is a good
...
%DIF > If one possesses some prior knowledge on the data generation process,
%DIF > another risk might be more adapted. This is typically the case for big causal
%DIF > effect ratio to baseline and low overlap where the $\tau\text{-risk}_{IPW}$
%DIF > should be better.


\DIFaddend To facilitate better model selection, we provide Python
...
models, from gradient boosting to convolutional \DIFdelbegin \DIFdel{neutral}\DIFdelend \DIFaddbegin \DIFadd{neural network}\DIFaddend , or language
models.
...
convincing for observational settings where both simple \DIFdelbegin \DIFdel{Mean Squared
    Error --MSE, }\DIFdelend \DIFaddbegin \DIFadd{MSE --}\DIFaddend $\mu\text{-risk}(f)$ def.\,\ref{def:mu_risk}-- and
...
\cite{schuler_comparison_2018}. \DIFaddbegin \DIFadd{An interesting direction is taken in \mbox{%DIFAUXCMD
        \cite{foster2023orthogonal} }\hskip0pt%DIFAUXCMD
    where the authors derive convergence rates for orthogonal losses such as the R-loss, or the DR-Loss without assuming well-specification of the model for target parameter.
}\DIFaddend
...
\DIFaddbegin \begin{assumption}[Consistency]\label{assumption:consistency} \DIFadd{The observed
        outcome is the potential outcome of the assigned treatment:
    }\begin{equation*}\DIFadd{\label{eq:consistancy}
            Y = A \, Y(1) + (1-A) \, Y(0)
        }\end{equation*}
    \DIFadd{Here, we assume that the intervention $A$ has been well defined. This
        ...
        intervention \mbox{%DIFAUXCMD
            \cite{hernan_causal_2020}}\hskip0pt%DIFAUXCMD
        .
    }\end{assumption}

\DIFaddend \begin{assumption}[Unconfoundedness]\label{assumption:ignorability}
...
\DIFdelbegin %DIFDELCMD < \begin{assumption}[Consistency]\label{assumption:consistency} %%%
\DIFdel{The observed
    outcome is the potential outcome of the assigned treatment:
}\begin{displaymath}\DIFdel{%DIFDELCMD < \label{eq:consistancy}%%%
        Y = A \, Y(1) + (1-A) \, Y(0)
...
\DIFdel{Here, we assume that the intervention $A$ has been well defined. This
    ...
    intervention \mbox{%DIFAUXCMD
        \cite{hernan_causal_2020}}\hskip0pt%DIFAUXCMD
    .
}%DIFDELCMD < \end{assumption}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \begin{assumption}[Generalization]\label{assumption:generalization} The training
...
\subsubsection{Reformulation of the $R\text{-risk}$ as reweighted
...
\begin{figure}
    \begin{subfigure}[b]{\textwidth}
        ...
        \includegraphics[width=\linewidth]{overlap_measure_diff_oracle_ntv_to_pred_ntv.pdf}
    \end{subfigure}
    \begin{subfigure}[b]{\textwidth}
        \centering
        \caption{\textbf{Calibrated
                classifiers}}
        \includegraphics[width=\linewidth]{overlap_measure_diff_oracle_ntv_to_ntv_wcalibration.pdf}
    \end{subfigure}
...
assumption for causal inference. \DIFaddbegin \DIFadd{Algorithm \ref{apd:experiments:generation:algo} describes the generation process for one simulation. }\DIFaddend Figure \ref{fig:simulation_examples}
...
\begin{itemize}
    ...
    \item A basis expansion of the raw features increases the problem dimension.
          ...
          (related to $\eta$ in \DIFdelbegin \DIFdel{Proposition
              \ref{theory:prop:mu_risk_ipw_bound}}\DIFdelend \DIFaddbegin \DIFadd{Assumption
              \ref{assumption:overlap}}\DIFaddend ). Finally, we generate observed
          ...
          of the full kernel of the data generation process \DIFdelbegin \DIFdel{$RBF(x,
                  \cdot) \;  with \; x \sim \mathbb P(x)$ }\DIFdelend with these
    ...
    \item Functions $\mu_0$, $\tau$ are distinct linear functions of the
          ...
          \end{equation*}
          \DIFaddbegin \DIFadd{Where $\beta_{\mu}$ and $\beta_{\tau}$ are sampled from two normal distributions with mean 0 and unit variances $\mathcal{N}(0, I_{D+1})$.
          }\DIFaddend \item Adding a Gaussian noise, $\varepsilon \sim \mathcal N(0, \sigma(x;a))$,
          we construct the potential outcomes:
          \DIFdelbegin \DIFdel{$y(a) = \mu_0(x) + a\,\tau(x) + \varepsilon(x, a)$
          }\DIFdelend \DIFaddbegin \DIFadd{$y(a) = (1-\omega)\mu_0(x) + a\,\omega\tau(x) + \varepsilon(x, a)$ with $\omega$ the treatment effect relative size with respect to the baseline response.
...
\DIFaddbegin \begin{algorithm}
    \caption{\DIFadd{Data simulation for the simulated dataset caussim}}
    \label{apd:experiments:generation:algo}
    \DIFadd{Let the parameters of a given simulation be:
    }\begin{itemize}
        \item \DIFadd{N the number of samples.
              }\item \DIFadd{D the dimension of the basis expansion.
              }\item \DIFadd{$p_A$ the proportion of treated individuals.
              }\item \DIFadd{$\theta$ the overlap parameter.
              }\item \DIFadd{%
                  \mbox{%DIFAUXCMD
                      $\mathbb P(x|A=a) = W \cdot \mathcal N \Big( \begin{bmatrix} (1-2a) \theta \\ 0\end{bmatrix} ; \begin{bmatrix} \sigma_0 & 0 \\ 0 & \sigma_1\end{bmatrix} \Big)$
                  }%DIFAUXCMD
                  the rotated gaussian for each population covariate. $W$ is a random rotation matrix and $\sigma_0^2=2;\sigma_1^2=5$ are scaling parameters.
              }\item \DIFadd{$e(x)=\mathbb P(A=1|X=x)$ the oracle propensity score, obtained analytically from $\mathbb P(x|A=a)$.
              }\item \DIFadd{$\mathbb P(X) = p_A \mathbb P(X|A=1) + (1- p_A) \mathbb P(X|A=0)$ the mixture of gaussian for the whole population covariates.
              }\item \DIFadd{$b_1, .., b_D \sim \mathbb P(X)$, the basis sampled from the gaussian mixture.
              }\item \DIFadd{$z(x) = [RBF_{\gamma}(x, b_d)]_{d=1..D} \cdot Z^T \in
              ...
              inverse of the basis kernel $Z=[K(b_i, b_j)]_{i, j \in
                  {1..D}}^{-1/2}$.
              }\item \DIFadd{$\beta_{\mu}$ and $\beta_{\tau}$, the linear coefficients on top of the basis expansion. They are sampled from two normal distributions with mean 0 and unit variances $\mathcal{N}(0, I_{D+1})$.
              }\item \DIFadd{$\omega$ the treatment effect relative size with respect to the baseline response.
              }\item \DIFadd{$\varepsilon(x, a) \sim \mathcal N(0, \sigma_y)$ an exogen gaussian noise with $\sigma_y$ the scale of the noise.
              }\end{itemize}

    \DIFadd{Generation process for one sample ($x$, $a$, e(x), $y(0)$, $y(1)$, $y$):
    }\begin{algorithmic}[1]
        \State \DIFadd{$x \sim \mathbb P(X)$ }\Comment{Sample from rotated gaussian mixture}
        \State  \DIFadd{$z(x)$ }\Comment{Compute the Nystroem expansion}
        \State \DIFadd{%
            \mbox{%DIFAUXCMD
                $\mu_0(x) = \begin{bmatrix} z(x); 1 \end{bmatrix} \cdot \beta_{\mu}^T$
            }%DIFAUXCMD
        }\Comment{Compute the non-treated response surface, ie. the baseline}
        \State  \DIFadd{%
            \mbox{%DIFAUXCMD
                $\tau(x) = \begin{bmatrix} z(x); 1 \end{bmatrix} \cdot \beta_{\tau}^T$
            }%DIFAUXCMD
        }\Comment{Compute the conditional treatment effect (CATE)}
        \State \DIFadd{$y(a) = (1-\omega) \mu_0(x) + a\,\omega\tau(x) + \varepsilon(x, a) + \varepsilon(x, a)$ }\Comment{Compute the potential outcomes}
        \State \DIFadd{$a \sim \operatorname{Bernoulli}(e(x))$ }\Comment{Compute the treatment status}
        \State \DIFadd{$y = a\,y(1) + (1-a)\,y(0)$ }\Comment{Compute the observed outcome}
    \end{algorithmic}
...
\begin{figure}[!b]
    ...
    %\hfill
    \caption{\DIFdelbeginFL \textbf{\DIFdelFL{The $R$-risk is the best metric}}%DIFAUXCMD
        \DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{The $R$-risk is the best metric on average}}\DIFaddendFL : Relative Kendall's $\tau$
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.44\textwidth}
        \centering
        \caption{\textbf{Caussim}}
        \includegraphics[width=\textwidth]{ktau_caussim_nuisance_nlin_models_ridge_ov1-247.pdf}
    ...
    \begin{subfigure}[b]{0.44\textwidth}
        \centering
        \caption{\textbf{ACIC 2016}}
        \includegraphics[width=\textwidth]{ktau_acic2016_nuisance_nlin_models_hgb_dgp1-77_rs1-10.pdf}
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.9\textwidth}
        \centering
        \caption{\textbf{Twins}}
        \includegraphics[width=1\textwidth]{_3_procedure_Twins_N11984__ref_oracle_r_risk.pdf}
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{_2_overlap_influence_by_bin_k_by_ds.pdf}
    \hfill
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        %\centering
        \caption{\textbf{Caussim}}
        \includegraphics[width=1.15\textwidth]{_4_nuisance_models_Caussim_N5000__ref_oracle_r_risk.pdf}
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \caption{\textbf{ACIC 2016}}
        \includegraphics[width=1\textwidth]{_4_nuisance_models_ACIC2016_N4802__ref_oracle_r_risk.pdf}
...
\DIFaddbegin \subsection{\DIFadd{Sensitivity of the results to different ratio of causal effect to baseline response}}\label{apd:effect_ratio}

\DIFadd{Going beyond overlap, we study the effect of another important parameter of the
    data generation process: the ratio of causal effect to the baseline response.
}

\DIFadd{Here, we detail the experimental setup of the results presented in Figure \ref{fig:effect_ratio_influence}.
}

\DIFadd{For each dataset of our simulation (caussim), we measure this ratio as the absolute mean
...
\begin{table}[!ht]
    ...
    \resizebox{\textwidth}{!}{ % Adjusts the table to fit the text width
        \begin{tabular}{|l|r|r|r|r|r|r|r|r|r|r|r|r|}
            \hline
                                   & \textbf{count} & \textbf{mean} & \textbf{std} & \textbf{min} & \textbf{1\%} & \textbf{10\%} & \textbf{25\%} & \textbf{50\%} & \textbf{75\%} & \textbf{90\%} & \textbf{99\%} & \textbf{max} \\ \hline
            \textbf{effect\_ratio} & 179            & 16.95         & 24.92        & 0.043        & 0.054        & 0.32          & 1.34          & 9.12          & 22.55         & 39.97         & 101.12        & 206.53       \\ \hline
        \end{tabular}
    }
    \caption{\DIFaddFL{Distribution of the causal effect ratio $\Delta_\mu$ for the experiment on causal effect ratio, results in Figure \ref{fig:effect_ratio_influence}.}}\label{apd:effect_ratio_distribution}
\end{table}


\DIFadd{We also studied another measure, focusing on the ratio of the variance of the causal effect to the variance of the mean shared response ($\mu_0 + \mu_1$) of both population: $\frac{\hat{var}(\mu_{1}(x_i) - \mu_{0}(x_i))}{\hat{var}(\mu_{1}(x_i) + \mu_{0}(x_i))}$, covering a range from 0 to 11.08 with the full
...
\begin{figure}[!tb]
    ...
    \caption{\textbf{\DIFaddFL{R-risk is robust to a wide range of effect variance}}\DIFaddFL{:
    Kendall's $\tau$ agreement with $\tau$-risk. Strong, medium and Weak Causal
...
\begin{table}[!ht]
    ...
    \resizebox{\textwidth}{!}{ % Adjusts the table to fit the text width
        \begin{tabular}{|l|r|r|r|r|r|r|r|r|r|r|r|r|}
            \hline
                                      & \textbf{count} & \textbf{mean} & \textbf{std} & \textbf{min} & \textbf{1\%} & \textbf{10\%} & \textbf{25\%} & \textbf{50\%} & \textbf{75\%} & \textbf{90\%} & \textbf{99\%} & \textbf{max} \\ \hline
            \textbf{effect\_variance} & 179            & 1.013         & 1.781        & 1.680e-5     & 1.734e-5     & 4.875e-4      & 6.449e-3      & 0.789         & 1.160         & 1.807         & 9.641         & 11.079       \\
            \hline
        \end{tabular}
    }
    \caption{\DIFaddFL{Statistical summary of the causal effect variance.}}
    \label{table:effect_variance}
\end{table}

\DIFaddend \bibliography{biblio}
